...
<div class="dashboard-container">
    ...
    <div class="stats-container-compact">
        ...
        <div class="stat-card-compact stat-gray">
            <div class="stat-icon-compact">
        ...
        <div class="stat-card-compact stat-indigo">
            <div class="stat-icon-compact">
                <i class="fas fa-percentage"></i>
            </div>
            <div class="stat-info-compact">
                <h3><?= $completed_tasks > 0 ? round(($completed_tasks / ($completed_tasks + $active_tasks)) * 100) : 0 ?>%</h3>
                <p>Success</p>
            </div>
        </div>
    </div>
    ...
    <div class="content-grid-compact">
        ...
        <div class="content-card-compact">
            <div class="card-header-compact">
                ...
                <a href="?page=todos" class="view-all-compact">Lihat Semua</a>
            </div>
            <div class="activity-list-compact">
                <?php if($recent_todos->num_rows > 0): ?>
                    <?php while($todo = $recent_todos->fetch_assoc()): ?>
                    <div class="activity-item-compact">
                        <div class="activity-icon-compact priority-<?= $todo['priority'] ?>">
                            <i class="fas fa-<?= getPriorityIcon($todo['priority']) ?>"></i>
                        </div>
                        <div class="activity-content-compact">
                            <h4><?= htmlspecialchars(substr($todo['title'], 0, 35)) ?><?= strlen($todo['title']) > 35 ? '...' : '' ?></h4>
                            <div class="activity-meta-compact">
                                <span><i class="fas fa-cube"></i><?= htmlspecialchars($todo['app_name']) ?></span>
                                <span><i class="fas fa-calendar"></i><?= date('d/m', strtotime($todo['created_at'])) ?></span>
                            </div>
                        </div>
                        <div class="task-status-compact status-<?= $todo['task_status'] ?>">
                            <?php if($todo['task_status'] == 'available'): ?>
                                <i class="fas fa-hand-paper"></i>
                            <?php elseif($todo['task_status'] == 'in_progress'): ?>
                                <i class="fas fa-clock"></i>
                            <?php else: ?>
                                <i class="fas fa-check-circle"></i>
                            <?php endif; ?>
        ...
        <div class="content-card-compact">
            <div class="card-header-compact">
                <h3><i class="fas fa-chart-bar"></i> Top Apps</h3>
                <a href="?page=apps" class="view-all-compact">Kelola</a>
            </div>
            <div class="app-performance-compact">
                <?php if($app_performance->num_rows > 0): ?>
                    <?php while($app = $app_performance->fetch_assoc()): ?>
                    <div class="app-perf-item-compact">
                        <div class="app-name-compact"><?= htmlspecialchars(substr($app['name'], 0, 20)) ?><?= strlen($app['name']) > 20 ? '...' : '' ?></div>
                        <div class="app-stats-mini-compact">
                            <span class="mini-stat-compact completed"><?= $app['completed'] ?></span>
                            <span class="mini-stat-compact progress"><?= $app['in_progress'] ?></span>
                            <span class="mini-stat-compact available"><?= $app['available'] ?></span>
                        </div>
                        <div class="progress-bar-mini-compact">
                            <div class="progress-fill-compact" style="width: <?= $app['total_todos'] > 0 ? ($app['completed'] / $app['total_todos']) * 100 : 0 ?>%"></div>
                        </div>
                    </div>
                    <?php endwhile; ?>
                <?php else: ?>
        ...
        <div class="content-card-compact">
            <div class="card-header-compact">
                ...
                <a href="?page=users" class="view-all-compact">Kelola</a>
            </div>
            <div class="user-distribution-compact">
                <?php if($user_stats->num_rows > 0): ?>
                    <?php while($stat = $user_stats->fetch_assoc()): ?>
                    <div class="role-item-compact">
                        <div class="role-info-compact">
                            <span class="role-name-compact">
                                <i class="<?= getRoleIcon($stat['role']) ?>"></i>
                                <?= ucfirst($stat['role']) ?>
                            </span>
                            <span class="role-count-compact"><?= $stat['count'] ?></span>
                        </div>
                        <div class="role-bar-compact">
                            <div class="role-progress-compact role-<?= $stat['role'] ?>" 
        ...
        <div class="content-card-compact">
            ...
            <div class="quick-actions-compact">
                <a href="?page=todos" class="quick-btn-compact btn-blue">
                    <i class="fas fa-plus"></i>
                    <span>Todo</span>
                </a>
                <?php if($_SESSION['user_role'] == 'admin' || $_SESSION['user_role'] == 'programmer'): ?>
                <a href="?page=apps" class="quick-btn-compact btn-green">
                    <i class="fas fa-cube"></i>
                    <span>Apps</span>
                </a>
                <?php endif; ?>
                <?php if($_SESSION['user_role'] == 'admin'): ?>
                <a href="?page=users" class="quick-btn-compact btn-orange">
                    <i class="fas fa-user-plus"></i>
                    <span>Users</span>
                </a>
                <?php endif; ?>
                <a href="?page=taken" class="quick-btn-compact btn-purple">
